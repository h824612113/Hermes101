...
- Day 3 之后，周沐的助手已经有稳定人格和执行边界。
- 今天进入执行层：把邮件、日历、搜索、浏览器接进 `TaskOrbit` 工作流。
...
| **邮件首响时长** | 8h40m | 3h10m |
| **会议准备提前量** | 0h | 1h00m |
| **SEO 异常发现时延** | 30h | 6h00m |

> 这组值来自周沐接入 Gmail/Calendar/Search 后的首周样例，说明“接入真实上下文”本身就能带来第一段收益。
...
> 💡 **实战备注**：我每天早上 8 点自动检查你的邮箱，把真正重要的挑出来发给周沐。其他的？GitHub 通知、账单、Newsletter……我先归类，不值得打扰。这才叫「助手」，不然和「新邮件提醒」没区别。
...
> 帮我打开 https://taskorbit.app 看看首页现在长什么样
...
> 💡 **实战备注**：浏览器是我最喜欢的技能之一。以前你让我查竞品，我只能搜索。现在我可以直接打开对方网站，像真人用户一样浏览。有一次我发现竞品悄悄改了定价策略，第一时间通知了周沐。他说这一个发现就值一个月服务器费用。
...
> 💡 **实战备注**：你有一次让我查一个同事的邮件记录，我拒绝了。因为 SOUL.md 里写了"只处理周沐自己的数据"。他当时有点惊讶，然后说"好，这条规则留着"。边界清晰，信任才稳固。
