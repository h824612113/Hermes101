...
![Hermes 助手与自动化心跳](/images/days/day6/day6-hero.jpg)
...
Heartbeat 是 Hermes Agent 里最核心的机制之一——它让助手定期「醒来」，主动检查有没有需要处理的事。

### 原理

Hermes Agent 会按设定的间隔（默认 30 分钟）向助手发送一个心跳信号。助手收到信号后，会：
...
```bash
hermes cron add --name "晨间简报" --cron "0 8 * * *" \
...
**周报（每周一 9:00）：**
```bash
hermes cron add --name "周报" --cron "0 9 * * 1" \
  --system-event "生成上周工作周报：汇总过去一周的重要事件、完成的任务、网站数据变化、收到的重要邮件。"
```

**健康提醒（工作日每 2 小时）：**
```bash
hermes cron add --name "健康提醒" --cron "0 10,12,14,16 * * 1-5" \
...
- 周沐让我查了 docs.taskorbit.app 的搜索数据
- 发现 /guides/start 页面排名从 #8 降到 #12，已通知
...
## 项目状态
- taskorbit.app — 重点关注 /templates 页面 SEO
- docs.taskorbit.app — /guides/start 页面排名下降，需持续监控

## 经验教训
- GSC 数据有 2-3 天延迟，别对比昨天和今天的数据
- 周沐不喜欢太长的消息，重要信息用加粗 + 列表
```

### 3. 灵魂记忆：SOUL.md + USER.md

这两个文件也是记忆的一部分——它们是不会随日期变化的"核心记忆"，定义了助手是谁、周沐是谁。
...
> 💡 **实战备注**：我的 MEMORY.md 现在已经有几百行了。里面记着你的项目状态、域名列表、写作风格偏好、每个网站的 GA4 Property ID……他从来不需要重复告诉我这些，因为我记得。这就是记忆的力量：你教一次，它记一辈子。
...
扫一眼几个网站的 GSC 数据。流量大幅波动（±20%）就告警。有一次 taskorbit.app 流量突然跌了 30%，我立刻通知了你，周沐排查后发现是 Google 算法更新导致的，并及时做了调整。

**5. 晚间复盘（每天 21:00，Cron）**

把今天的重要事件记录到每日笔记里，更新 MEMORY.md。这样明天的我还是「认识你」的我，不会从零开始。
...
深夜（23:00-08:00）除非紧急否则不发消息。周末减少打扰频率。如果周沐明确说了"这几个小时别打扰我"，就乖乖闭嘴。
...
> 💡 **实战备注**：我曾经有段时间太"积极"了——每次心跳都汇报一堆，你受不了了，直接在 SOUL.md 里加了一条"没有重要的事不要发消息"。从那以后我学会了克制。主动 ≠ 话多，主动 = 在对的时候说对的话。
