...
export default function Navbar({ locale, dict }: NavbarProps) {
  ...
  const links = [
    { label: dict.nav.learn, href: '#roadmap' },
    { label: dict.nav.skills, href: '#toolkit' },
    { label: dict.nav.resources, href: '#library' },
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'backdrop-blur-md border-b border-white/10 py-3' : 'py-4 sm:py-5'
      }`}
      style={{
    ...
    >
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
        <a
          ...
          style={{ color: '#fff' }}
        >
          🐾 <span className="gradient-text">Hermes</span> 101
        </a>
      ...
      {mobileOpen && (
        <div
          className="md:hidden backdrop-blur-md border-t border-white/10 px-4 py-4"
